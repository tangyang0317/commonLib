...
import com.bqj.mall.library.commonlib.view.sku.utils.ViewUtils;
import com.bqj.mall.library.commonlib.view.sku.widget.SkuMaxHeightScrollView;
...
public class SkuSelectScrollView extends SkuMaxHeightScrollView implements SkuItemLayout.OnSkuItemSelectListener {
     ...
     * @param skuList
     */
    public void setSkuList(List<Sku> skuList, boolean clickAble) {
        ...
        for (Iterator<Map.Entry<String, List<String>>> it = dataMap.entrySet().iterator(); it.hasNext(); ) {
            ...
            itemLayout.buildItemLayout(index++, entry.getKey(), entry.getValue(), clickAble);
            if (clickAble) {
    ...
    private Map<String, List<String>> getSkuGroupByName(List<Sku> list) {
        ...
        for (Sku sku : list) {
            for (SkuAttribute attribute : sku.getAttr()) {
    ...
    private void optionLayoutEnableStatusSingleProperty() {
        ...
        for (int i = 0; i < skuList.size(); i++) {
            ...
            List<SkuAttribute> attributeBeanList = skuList.get(i).getAttr();
    ...
    private void optionLayoutEnableStatusMultipleProperties() {
        for (int i = 0; i < skuContainerLayout.getChildCount(); i++) {
            ...
            for (int j = 0; j < skuList.size(); j++) {
                ...
                List<SkuAttribute> attributeBeanList = sku.getAttr();
                // 遍历选中信息列表
    ...
    public Sku getSelectedSku() {
        ...
        for (Sku sku : skuList) {
            List<SkuAttribute> attributeList = sku.getAttr();
